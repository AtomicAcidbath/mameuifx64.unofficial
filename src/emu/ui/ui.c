	...
	if (m_mouse_show || (is_menu_active() && machine().options().ui_mouse()))
	{
		...
		if (mouse_target != NULL)
		{
			...
			if (mouse_target->map_point_container(mouse_target_x, mouse_target_y, *container, mouse_x, mouse_y)) {
				container->add_quad(mouse_x,mouse_y,mouse_x + 0.05*container->manager().ui_aspect(),mouse_y + 0.05,ARGB_WHITE,m_mouse_arrow_texture,PRIMFLAG_BLENDMODE(BLENDMODE_ALPHA));
			}
		}
	}
...
astring &ui_manager::warnings_string(astring &string)
{
...
astring &ui_manager::game_info_astring(astring &string)
{
