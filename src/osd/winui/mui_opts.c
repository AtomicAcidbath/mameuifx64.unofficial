...
#define MUIOPTION_EXTRA_DIRECTORY				"extra_directory"
...
#define MUIOPTION_UI_KEY_VIEW_TAB_EXTRA 		"ui_key_view_tab_extra"
...
const options_entry winui_options::s_option_entries[] =
{
	...
	{ MUIOPTION_WINDOW_HEIGHT,				"638",      OPTION_INTEGER,  	NULL },
	...
	{ MUIOPTION_COLUMN_WIDTHS,				"320,80,80,140,50,160,60,70", 		OPTION_STRING, NULL },
	...
	{ MUIOPTION_EXTRA_DIRECTORY,			"extras",   OPTION_STRING,    NULL },
    ...
    { MUIOPTION_UI_KEY_VIEW_TAB_EXTRA,		"KEYCODE_LALT KEYCODE_8",     OPTION_STRING, NULL },
...
{
	astring error_string;
	core_opts.set_value(OPTION_FONTPATH, paths, OPTION_PRIORITY_CMDLINE, error_string);
	assert(!error_string);
}
...
void SetCheatDir(const char* paths)
{
	astring error_string;
	core_opts.set_value(OPTION_CHEATPATH, paths, OPTION_PRIORITY_CMDLINE, error_string);
	assert(!error_string);
}
...
{
	astring error_string;
	core_opts.set_value(OPTION_CROSSHAIRPATH, paths, OPTION_PRIORITY_CMDLINE, error_string);
	assert(!error_string);
}
...
const char * GetExtraDir(void)
{
	return gui_opts.value(MUIOPTION_EXTRA_DIRECTORY);
}

void SetExtraDir(const char *path)
{
	astring error_string;
	gui_opts.set_value(MUIOPTION_EXTRA_DIRECTORY, path, OPTION_PRIORITY_CMDLINE, error_string);
	assert(!error_string);
}
...
{
	return core_opts.value(OPTION_SNAPNAME);
}

void SetSnapName(const char* pattern)
{
	astring error_string;
	core_opts.set_value(OPTION_SNAPNAME, pattern, OPTION_PRIORITY_CMDLINE, error_string);
	assert(!error_string);
}
...
input_seq* Get_ui_key_view_tab_extra(void)
{
	return options_get_input_seq(gui_opts, MUIOPTION_UI_KEY_VIEW_TAB_EXTRA);
}
...
{
	return gui_opts.value(MUIOPTION_EXEC_COMMAND);
}

void SetExecCommand(char *cmd)
{
	astring error_string;
	gui_opts.set_value(MUIOPTION_EXEC_COMMAND, cmd, OPTION_PRIORITY_CMDLINE, error_string);
	assert(!error_string);
}

int GetExecWait(void)
{
	return gui_opts.int_value(MUIOPTION_EXEC_WAIT);
}

void SetExecWait(int wait)
{
	astring error_string;
	gui_opts.set_value(MUIOPTION_EXEC_WAIT, wait, OPTION_PRIORITY_CMDLINE, error_string);
	assert(!error_string);
}
...
{
	// parse GAMELIST.INI
	astring gamename (GetGuiDir(), PATH_SEPARATOR, GAMELIST_INI_FILENAME, ".ini");
	game_opts.load_file(gamename);

	// parse INTERFACE.INI
	astring intername (GetGuiDir(), PATH_SEPARATOR, INTERFACE_INI_FILENAME, ".ini");
	LoadInterfaceFile(gui_opts, intername);

	// parse DIRECTORIES.INI
	astring filename (GetGuiDir(), PATH_SEPARATOR, DIRECTORIES_INI_FILENAME, ".ini");
	LoadOptionsFile(core_opts, filename);
}
...
void LoadFolderFlags(void)
{
	...
	astring filename (GetGuiDir(), PATH_SEPARATOR, INTERFACE_INI_FILENAME, ".ini");
...
void SaveInterface(void)
{
	...
	astring filename (GetGuiDir(), PATH_SEPARATOR, INTERFACE_INI_FILENAME, ".ini");
	SaveInterfaceFile(gui_opts, NULL, filename);
}

void SaveDirectories(void)
{
	astring filename (GetGuiDir(), PATH_SEPARATOR, DIRECTORIES_INI_FILENAME, ".ini");
	SaveOptionsFile(core_opts, NULL, filename);
}

void SaveGameList(void)
{
	astring filename (GetGuiDir(), PATH_SEPARATOR, GAMELIST_INI_FILENAME, ".ini");
	game_opts.save_file(filename);
}
...
void SaveOptions(OPTIONS_TYPE opt_type, windows_options &opts, int game_num)
{
	...
	if (opt_type == OPTIONS_GLOBAL)
	{
		// Don't try to save a null core_opts options file,  or it will be erased.
		//if (NULL == opts)
			//return;
		filename.cpy(DEFAULT_INI_FILENAME);
	} 
	...
	if (filename)
	{
		...
		SetDirectories(opts);
		SaveOptionsFile(opts, NULL, filepath);
	}
}
