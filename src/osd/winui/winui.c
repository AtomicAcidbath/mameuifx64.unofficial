...
/* Last list directory for Save Game or ROMs List dialogs */
static TCHAR list_directory[MAX_PATH];
...
#define WM_MAME32_FILECHANGED	(WM_USER + 1)
#define WM_MAME32_AUDITGAME		(WM_USER + 2)
...
static const TBBUTTON tbb[] =
{
	...
	{0, 0,                   	TBSTATE_ENABLED, BTNS_SEP,        {0, 0}, 0, 0},
	{9, ID_PLAY_VIDEO,			TBSTATE_ENABLED, BTNS_BUTTON,     {0, 0}, 0, 7},
...
/* system-wide window message sent out with an ATOM of the current game name each time it changes */
...
int MameUIMain(HINSTANCE hInstance, LPWSTR lpCmdLine)
{
	...
	g_mame32_message = RegisterWindowMessage(TEXT("MAMEUIFX"));
	...
	memset (&MameIcon, 0, sizeof(MameIcon));
	...
	swprintf(MameIcon.szInfoTitle, TEXT("MAMEUIFX"));
	swprintf(MameIcon.szInfo, TEXT("Still running...."));
	swprintf(MameIcon.szTip, TEXT("MAMEUIFX"));
...
static void ResizeTreeAndListViews(BOOL bResizeHidden)
{
	...
	for (i = 0; g_splitterInfo[i].nSplitterWindow; i++)
	{
		...
		if (bResizeHidden || bVisible)
		{
			...
			MoveWindow(GetDlgItem(hMain, g_splitterInfo[i].nLeftWindow), nLastWidth, rect.top + 3,
				nLeftWindowWidth, (rect.bottom - rect.top) - 8, TRUE);

			MoveWindow(GetDlgItem(hMain, g_splitterInfo[i].nSplitterWindow), nSplitterOffset[i], rect.top + 3,
				SPLITTER_WIDTH, (rect.bottom - rect.top) - 8, TRUE);
		}
...
void UpdateScreenShot(void)
{
	...
	if (GetShowScreenShot())
	{
		...
		MoveWindow(GetDlgItem(hMain, IDC_SSPICTURE), fRect.left + rect.left, fRect.top + rect.top,
...
void ResizePickerControls(HWND hWnd)
{
	...
	MoveWindow(GetDlgItem(hWnd, IDC_DIVIDER), rect.left, rect.top - 3, rect.right, 1, TRUE);

	ResizeTreeAndListViews(TRUE);

	nListWidth = nSplitterOffset[nSplitterCount - 1];
	nScreenShotWidth = (rect.right - nListWidth);

	/* Screen shot Page tab control */
	if (bShowTabCtrl)
	{
		MoveWindow(GetDlgItem(hWnd, IDC_SSTAB), nListWidth + 2, rect.top + 3,
			nScreenShotWidth - 4, (rect.bottom - rect.top) - 7, doSSControls);
		rect.top += 21;
	}
	else
	{
		MoveWindow(GetDlgItem(hWnd, IDC_SSBORDER), nListWidth + 2, rect.top + 3,
			nScreenShotWidth - 6, (rect.bottom - rect.top) - 8, doSSControls);
	}
	...
	MoveWindow(GetDlgItem(hWnd, IDC_HISTORY), sRect.left, sRect.top,
...
int GetGameNameIndex(const char *name)
{
	...
	driver_index_info = (driver_data_type *)bsearch(&key, sorted_drivers, driver_list::total(), sizeof(driver_data_type), DriverDataCompareFunc);
...
int CLIB_DECL SrcDriverDataCompareFunc(const void *arg1, const void *arg2)
{
...
static void winui_output_error(delegate_late_bind *param, const char *format, va_list argptr)
{
	...
	win_message_box_utf8(win_window_list ? win_window_list->m_hwnd : NULL, buffer, emulator_info::get_appname(), MB_ICONERROR | MB_OK);
}

static void memory_error(const char *message)
{
	win_message_box_utf8(hMain, message, emulator_info::get_appname(), MB_ICONERROR | MB_OK);
	exit(-1);
}
...
static BOOL Win32UI_init()
{
	...
	for (i = 0; i < driver_list::total(); i++)
	{
		sorted_srcdrivers[i].name = driver_list::driver(i).source_file+17;
    ...
    */
	...
	{
		(void)ListView_SetExtendedListViewStyle(hWndList, LVS_EX_FULLROWSELECT | LVS_EX_HEADERDRAGDROP | 
											LVS_EX_LABELTIP | LVS_EX_ONECLICKACTIVATE | LVS_EX_DOUBLEBUFFER);
		SendMessage(hTreeView, TVM_SETEXTENDEDSTYLE, TVS_EX_DOUBLEBUFFER, TVS_EX_DOUBLEBUFFER);
	}
	else
		(void)ListView_SetExtendedListViewStyle(hWndList, LVS_EX_FULLROWSELECT | LVS_EX_HEADERDRAGDROP | LVS_EX_LABELTIP |
											LVS_EX_UNDERLINEHOT | LVS_EX_ONECLICKACTIVATE | LVS_EX_DOUBLEBUFFER);	
	...
	int status = GetWindowState();
	
	if (status == SW_HIDE || status == SW_MINIMIZE || status == SW_SHOWMINIMIZED)
	{
		status = SW_RESTORE;
	}
	...
	if (validity_failed)
	{
		win_message_box_utf8(hMain, MAMENAME " has failed its validity checks.  The GUI will\n"
			"still work, but emulations will fail to execute", MAMENAME, MB_ICONERROR | MB_OK);
	}
...
static LRESULT CALLBACK MameWindowProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CTLCOLORSTATIC:
		...
		{
			SetBkColor(hDC, GetHistoryBgColor());
	...
	case WM_CLOSE:
		{
			...
			if (state == SW_MINIMIZE || state == SW_SHOWMINIMIZED || state == SW_HIDE)
			{
				...
				{
					state = SW_RESTORE;
					//ShowWindow(hWnd, SW_RESTORE); 	// do not restore now, we'll do at next startup!!
				}
			}
...
static BOOL OnIdle(HWND hWnd)
{
	...
	pDescription = driver_list::driver(driver_index).description;
	SetStatusBarText(0, pDescription);
	pName = driver_list::driver(driver_index).name;
...
static void ProgressBarShow()
{
	...
	SendMessage(hProgWnd, PBM_SETRANGE, 0, (LPARAM)MAKELONG(0, game_total));
	SendMessage(hProgWnd, PBM_SETPOS, 0, 0);

	StatusBar_GetItemRect(hStatusBar, 1, &rect);

	MoveWindow(hProgWnd, rect.left, rect.top + 1, rect.right - rect.left, rect.bottom - rect.top - 3, TRUE);
...
static void ProgressBarHide()
{
	...
	GetTextExtentPoint32(hDC, (TCHAR*)MAMEUINAME, 8, &size);
	widths[4] = size.cx;
	GetTextExtentPoint32(hDC, TEXT("XXXXXXXXXXXX"), 10, &size);
	widths[3] = size.cx;
	GetTextExtentPoint32(hDC, TEXT("XXXXXXXXXXXXXXXXXXXXXXXX"), 16, &size);
	widths[2] = size.cx;
	GetTextExtentPoint32(hDC, TEXT("XXXXXXXXXXXX"), 12, &size);
	widths[1] = size.cx;
	...
	widths[0] = (rect.right - rect.left) - (widths[1] + widths[2] + widths[3] + widths[4]);
	widths[1] += widths[0];
	widths[2] += widths[1];
	widths[3] += widths[2];
	widths[4] += widths[3];
...
static void InitProgressBar(void)
{
	...
	hProgWnd = CreateWindowEx(0, PROGRESS_CLASS, TEXT("Progress Bar"), PBS_SMOOTH | WS_CHILD | WS_CLIPSIBLINGS,
					rect.left, rect.top + 1, rect.right - rect.left, rect.bottom - rect.top - 3, hStatusBar, NULL, hInst, NULL);
}
...
static void InitToolbar(void)
{
	...
	HIMAGELIST hToolList = NULL;
    RECT rect;
	int idx;
	int iPosX, iPosY, iHeight;

	hToolBar = CreateWindowEx(WS_EX_COMPOSITED, TOOLBARCLASSNAME, TEXT("Toolbar"), WS_CHILD | WS_CLIPSIBLINGS | WS_VISIBLE | TBSTYLE_FLAT | TBSTYLE_TOOLTIPS,
				0, 0, NUM_TOOLBUTTONS * 32, 32, hMain, NULL, hInst, NULL);

	hBitmap = (HBITMAP)LoadImage(GetModuleHandle(NULL), MAKEINTRESOURCE(IDB_TOOLBAR), IMAGE_BITMAP, 0, 0, LR_SHARED);
    hToolList = ImageList_Create(32, 32, ILC_COLORDDB | ILC_MASK, NUM_TOOLBUTTONS, 0);	
	ImageList_AddMasked(hToolList, hBitmap, RGB(0, 0, 0));
	DeleteObject(hBitmap);

    SendMessage(hToolBar, TB_BUTTONSTRUCTSIZE, (WPARAM)sizeof(TBBUTTON), 0);
    SendMessage(hToolBar, TB_SETIMAGELIST, 0, (LPARAM)hToolList);
    SendMessage(hToolBar, TB_ADDBUTTONS, (WPARAM)NUM_TOOLBUTTONS, (LPARAM)&tbb);
	...
	iPosX = rect.right + 8;
	iPosY = (rect.bottom - rect.top) / 4;
	iHeight = rect.bottom - rect.top - 17;

	// create Search Edit Control
	hSearchWnd = CreateWindowEx(WS_EX_COMPOSITED, TEXT("Edit"), TEXT(SEARCH_PROMPT), ES_LEFT | WS_CHILD | WS_CLIPSIBLINGS | WS_BORDER | WS_VISIBLE,
...
{
	HBITMAP hBitmap = NULL;
	HIMAGELIST hTabViewList = NULL;

	hTabCtrl = CreateWindowEx(WS_EX_COMPOSITED, WC_TABCONTROL, TEXT("TabView"), WS_CHILD | WS_CLIPSIBLINGS | WS_VISIBLE | WS_TABSTOP | TCS_HOTTRACK,
				0, 0, 0, 0, hMain, (HMENU)IDC_SSTAB, hInst, NULL);

	hBitmap = (HBITMAP)LoadImage(GetModuleHandle(NULL), MAKEINTRESOURCE(IDB_TABVIEW), IMAGE_BITMAP, 0, 0, LR_SHARED);
    hTabViewList = ImageList_Create(16, 16, ILC_COLORDDB | ILC_MASK, MAX_TAB_TYPES, 0);	
...
static void InitStatusBar(void)
{
	...
	hStatusBar = CreateWindowEx(WS_EX_COMPOSITED, STATUSCLASSNAME, TEXT("Statusbar"), WS_CHILD | WS_CLIPSIBLINGS | WS_VISIBLE | SBARS_SIZEGRIP,
				0, 0, 0, 0, hMain, (HMENU)2, hInst, NULL);
}
...
static void UpdateStatusBar()
{
	...
	SetStatusBarTextF(3, g_szGameCountString, games_shown);
	...
	else
	{
		...
		SetStatusBarText(4, MAMEUINAME);
		SendMessage(hStatusBar, SB_SETICON, 4, (LPARAM)hIconFX);
	}
}
...
static void EnableSelection(int nGame)
{
	...
	t_description = tstring_from_utf8(ConvertAmpersandString(driver_list::driver(nGame).description));
	if( !t_description )
		return;
	...
	pText = driver_list::driver(nGame).description;
	SetStatusBarText(0, pText);
	pText = driver_list::driver(nGame).name;
	SetStatusBarText(1, pText);
	SendMessage(hStatusBar, SB_SETICON, 1, (LPARAM)GetSelectedPickItemIconSmall());
	/* Add this game's status to the status bar */
	pText = GameInfoStatusBar(nGame);
	SetStatusBarText(2, pText);
...
{
 	COLORREF cHistoryColor;

 	cHistoryColor = GetHistoryBgColor();
...
static BOOL MameCommand(HWND hWnd,int id, HWND hWndCtl, UINT codeNotify)
{
	...
	{
		(void)DialogBoxParam(GetModuleHandle(NULL), MAKEINTRESOURCE(IDD_CUSTOM_FILE),
	...
	case ID_MAME_HOMEPAGE:
		{
			ShellExecuteCommon(GetMainWindow(), "http://www.mamedev.org");
			return TRUE;
		}
		break;
	...
	case ID_MAME_FAQ:
		{
			ShellExecuteCommon(GetMainWindow(), "http://mamedev.org/devwiki/index.php?title=Frequently_Asked_Questions"); 
			return TRUE;
		}
		break;
	...
	case ID_PLAY_VIDEO:
		{
			...
			astring videoplay(GetVideoDir(), PATH_SEPARATOR, driver_list::driver(nGame).name, ".avi");
			ShellExecuteCommon(GetMainWindow(), videoplay);
			return TRUE;
		}
		break;
	...
	case ID_PLAY_AUDIO:
		{
			int nGame = Picker_GetSelectedItem(hWndList);
			astring audioplay(GetAudioDir(), PATH_SEPARATOR, driver_list::driver(nGame).name, ".wav");
			ShellExecuteCommon(GetMainWindow(), audioplay);
			return TRUE;
		}
		break;
	...
	case ID_PLAY_MNG:
		{
			int nGame = Picker_GetSelectedItem(hWndList);
			astring mngplay(GetVideoDir(), PATH_SEPARATOR, driver_list::driver(nGame).name, ".mng");
			ShellExecuteCommon(GetMainWindow(), mngplay);
			return TRUE;
		}
		break;
	...
	case ID_VIEW_ZIP:
		{
			int nGame = Picker_GetSelectedItem(hWndList);
			astring viewzip(GetRomDirs(), PATH_SEPARATOR, driver_list::driver(nGame).name, ".zip");
			ShellExecuteCommon(GetMainWindow(), viewzip);
			return TRUE;
		}
		break;
	...
	case ID_PLAY_M1:
		{
			...
			astring command(".\\m1\\m1fx.exe ", driver_list::driver(nGame).name);
			...
			{
				strcpy(error, "Selected game is missing ROMs!\nM1FX cannot be executed!");
				win_message_box_utf8(GetMainWindow(), error, "M1FX", MB_ICONERROR | MB_OK);
				return TRUE;
			}	
		}
		break;
	...
	case ID_CONTEXT_RENAME_CUSTOM :
		SendMessage(hTreeView, TVM_EDITLABEL, 0, (LPARAM)TreeView_GetSelection(hTreeView));		
		break;
		...
		for (i = 0; g_helpInfo[i].nMenuItem > 0; i++)
		{
			if (g_helpInfo[i].nMenuItem == id)
			{
				if (g_helpInfo[i].bIsHtmlHelp)
					HelpFunction(GetMainWindow(), g_helpInfo[i].lpFile, HH_DISPLAY_TOPIC, 0);
				else
					ShellExecuteCommon(GetMainWindow(), g_helpInfo[i].lpFile);
				return FALSE;
			}
		}
		break;
	...
	default:
		if (id >= ID_SHOW_FOLDER_START1 && id <= ID_SHOW_FOLDER_START26)
		{
...
static const TCHAR *GamePicker_GetItemString(HWND hwndPicker, int nItem, int nColumn, TCHAR *pszBuffer, UINT nBufferLength)
{
	...
	switch(nColumn)
	{
		...
		case COLUMN_PLAYED:
			/* played count */
			_sntprintf(pszBuffer, nBufferLength, TEXT("%d"), GetPlayCount(nItem));
			s = pszBuffer;
			break;
...
static void CreateIcons(void)
{
	...
	if (NULL == hSmall) 
	{
		win_message_box_utf8(hWndList, "Cannot allocate small icon image list", "Allocation error - Exiting", MB_ICONERROR | MB_OK);
		PostQuitMessage(0);
	}
	...
	if (NULL == hLarge) 
	{
		win_message_box_utf8(hWndList, "Cannot allocate large icon image list", "Allocation error - Exiting", MB_ICONERROR | MB_OK);
		PostQuitMessage(0);
	}
...
BOOL CommonFileDialog(common_file_dialog_proc cfd, char *filename, int filetype)
{
	...
	case FILETYPE_INPUT_FILES :
		of.lpstrFilter   = TEXT(MAMENAME) TEXT(" input files (*.inp,*.zip)\0*.inp;*.zip\0");
		break;
	case FILETYPE_SAVESTATE_FILES :
		of.lpstrFilter   = TEXT(MAMENAME) TEXT(" savestate files (*.sta)\0*.sta;\0");
		break;
	...
	case FILETYPE_JOYMAP_FILES :
		of.lpstrFilter   = TEXT("maps (*.map,*.txt)\0*.map;*.txt;\0");
		break;
	case FILETYPE_DEBUGSCRIPT_FILES :
		of.lpstrFilter   = TEXT("scripts (*.txt,*.dat)\0*.txt;*.dat;\0");
		break;
	}
	...
	else if (filetype == FILETYPE_WAVE_FILES)
	{
		t_audiodir = tstring_from_utf8(GetAudioDir());
		if( !t_audiodir )
			return FALSE;

		of.lpstrInitialDir = t_audiodir;
	}
	else if (filetype == FILETYPE_MNG_FILES || filetype == FILETYPE_AVI_FILES)
	{
		t_videodir = tstring_from_utf8(GetVideoDir());
		if( !t_videodir )
			return FALSE;

		of.lpstrInitialDir = t_videodir;
	}
	else 
	{
	...
	of.lpstrTitle        = NULL;
	of.Flags             = OFN_SHOWHELP | OFN_EXPLORER | OFN_PATHMUSTEXIST | OFN_HIDEREADONLY | OFN_ENABLEHOOK;
	of.nFileOffset       = 0;
	of.nFileExtension    = 0;
	switch (filetype)
	{
...
void InitMainMenu(HMENU hMainMenu)
{
	...
	hFile = GetSubMenu(hMainMenu, 0);
	hView = GetSubMenu(hMainMenu, 1);
	hOption = GetSubMenu(hMainMenu, 2);
	hTools = GetSubMenu(hMainMenu, 3);
	hAbout = GetSubMenu(hMainMenu, 4);
	hSubSort = GetSubMenu(hView, 9);
	hSubFold = GetSubMenu(hView, 11);
	hSubView = GetSubMenu(hView, 13);
	hSubFonts = GetSubMenu(hOption, 4);
	...
	SetMenuItemBitmaps(hMainMenu, ID_CONTEXT_ADD_CUSTOM, MF_BYCOMMAND, hCustom, hCustom);
	...
	SetMenuItemBitmaps(hMainMenu, ID_VIEW_ZIP, MF_BYCOMMAND, hZip, hZip);
	...
	SetMenuItemBitmaps(hMainMenu, ID_GAME_PROPERTIES, MF_BYCOMMAND, hProperties, hProperties);
	...
	SetMenuItemBitmaps(hView, 11, MF_BYPOSITION, hFolders, hFolders);
	SetMenuItemBitmaps(hView, 9, MF_BYPOSITION, hSort, hSort);
	SetMenuItemBitmaps(hMainMenu, ID_MAME_FAQ, MF_BYCOMMAND, hFaq, hFaq);
	SetMenuItemBitmaps(hView, 13, MF_BYPOSITION, hTabs, hTabs);
	SetMenuItemBitmaps(hMainMenu, ID_HELP_TROUBLE, MF_BYCOMMAND, hTrouble, hTrouble);
	SetMenuItemBitmaps(hMainMenu, ID_CONTEXT_RESET_PLAYCOUNT, MF_BYCOMMAND, hCount, hCount);
	SetMenuItemBitmaps(hMainMenu, ID_HELP_WHATS_NEW, MF_BYCOMMAND, hRelease, hRelease);
	SetMenuItemBitmaps(hMainMenu, ID_CONTEXT_RESET_PLAYTIME, MF_BYCOMMAND, hTime, hTime);
	...
	SetMenuItemBitmaps(hMainMenu, ID_CONTEXT_REMOVE_CUSTOM, MF_BYCOMMAND, hRemove, hRemove);
...
void InitTreeContextMenu(HMENU hTreeMenu)
{
	...
	hMenuTree = GetSubMenu(hTreeMenu, 0);
...
static void RemoveGameCustomFolder(int driver_index)
{
	...
	win_message_box_utf8(GetMainWindow(), "Error searching for custom folder", MAMEUINAME, MB_ICONERROR | MB_OK);
...
static BOOL CALLBACK StartupProc(HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_INITDIALOG:
		...
		SendDlgItemMessage(hDlg, IDC_SPLASH, STM_SETIMAGE, (WPARAM)IMAGE_BITMAP, (LPARAM)hBmp);
		hBrush = GetSysColorBrush(COLOR_3DFACE);
		EnableWindow(GetDlgItem(hSplash, IDC_PROGBAR), FALSE);
        hProgress = CreateWindowEx(0, PROGRESS_CLASS, TEXT("Progress"), PBS_SMOOTH | WS_CHILD | WS_VISIBLE, 0, 136, 526, 20, hDlg, NULL, hInst, NULL);
		LONG_PTR style = GetWindowLongPtr(hProgress, GWL_STYLE);
		SetWindowLongPtr(hProgress, GWL_STYLE, style | PBS_MARQUEE);		
		SendMessage(hProgress, PBM_SETMARQUEE, 1, 30);
		hStartupThread = CreateThread(NULL, 0, StartupThread, hDlg, 0, 0);
		SetThreadPriority(hStartupThread, THREAD_PRIORITY_IDLE);
		break;
		}
...
static BOOL CommonListDialog(common_file_dialog_proc cfd, int filetype)
{
	...
	while( !success )
	{
		if ( GetSaveFileName(&of) )
		{
			...
			{
				if ( win_message_box_utf8(GetMainWindow(), "File already exists, overwrite ?", 
					MAMEUINAME, MB_ICONQUESTION | MB_YESNO) != IDYES )
					continue;
				else
					success = TRUE;
			...
			{
				case FILETYPE_GAME_LIST :
					SaveGameListToFile(utf8_from_tstring(szFile));
					break;
				case FILETYPE_ROMS_LIST :
					SaveRomsListToFile(utf8_from_tstring(szFile));
					break;
			}
...
static void SaveGameListToFile(char *szFile)
{
    	...
    	if (fp == NULL)
    	{
        	win_message_box_utf8(GetMainWindow(), "Error : unable to access file", MAMEUINAME, MB_ICONERROR | MB_OK);
        	return;
    	}
    	...
    	sprintf( Buf, "This is the current list of games.%s", CrLf );
    	fwrite( Buf, strlen(Buf), 1, fp);
    	...
    	if ( GetSortColumn() > 0 )
    	{
        	sprintf( Buf, "Sorted by <%s> descending order", utf8_from_tstring(column_names[GetSortColumn()]) );
    	}
    	else
    	{
        	sprintf( Buf, "Sorted by <%s> ascending order", utf8_from_tstring(column_names[-GetSortColumn()]) );
    	}
    	...
    	for ( nIndex=0; nIndex<nListCount; nIndex++ )
    	{
        	...
        	if ( ListView_GetItem(hWndList, &lvi) )
        	{
            	...
            	for (i=0; i<nColumnMax; i++ )
            	{
                	...
                	{
                    case  0: 	// Name
						sprintf( &Buf[strlen(Buf)], "%s", driver_list::driver(nGameIndex).description );
                        break;
                	}
            	}
            	strcat( Buf, "\n");
    	...
    	SendMessage(hStatusBar, SB_SETTEXT, (WPARAM)0, (LPARAM)TEXT("File saved successfully"));}
...
static HBITMAP CreateBitmapTransparent(HBITMAP hSource)
{
	...
	hNew = CreateBitmap(bm.bmWidth, bm.bmHeight, bm.bmPlanes, bm.bmBitsPixel, NULL);
	SelectObject(hDst, hNew);
	
    BitBlt(hDst, 0, 0, bm.bmWidth, bm.bmHeight, hSrc, 0, 0, SRCCOPY);
 
    clrTP = RGB(239, 239, 239);
