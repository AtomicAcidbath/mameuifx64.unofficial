...
static BOOL WINAPI DDEnumInfo(GUID FAR *lpGUID, LPTSTR lpDriverDescription, LPTSTR lpDriverName, LPVOID lpContext, HMONITOR hm);
...
BOOL DirectDraw_Initialize(void)
{
     ...
     */
	pDDEnumEx = (LPDIRECTDRAWENUMERATEEX) GetProcAddress((HINSTANCE)g_hDLL, (LPCSTR)DirectDrawEnumerateExW);
