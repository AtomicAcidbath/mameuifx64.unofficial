...
typedef BOOL (WINAPI *READDIRECTORYCHANGESFUNC)(HANDLE hDirectory, LPVOID lpBuffer, DWORD nBufferLength, BOOL bWatchSubtree, DWORD dwNotifyFilter,
		LPDWORD lpBytesReturned, LPOVERLAPPED lpOverlapped, LPOVERLAPPED_COMPLETION_ROUTINE lpCompletionRoutine);
...
static BOOL DirWatcher_WatchDirectory(PDIRWATCHER pWatcher, int nIndex, int nSubIndex, LPCSTR pszPath, BOOL bWatchSubtree)
{
	...
	hDir = win_create_file_utf8(pszPath, FILE_LIST_DIRECTORY, FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE,
...
static void DirWatcher_Signal(PDIRWATCHER pWatcher, struct DirWatcherEntry *pEntry)
{
	...
	if (pWatcher->hWndTarget)
	{
		...
		SendMessage(pWatcher->hWndTarget, pWatcher->nMessage, (pEntry->nIndex << 16) | (pEntry->nSubIndex << 0),
...
PDIRWATCHER DirWatcher_Init(HWND hWndTarget, UINT nMessage)
{
	...
	pWatcher->hThread = CreateThread(NULL, 0, DirWatcher_ThreadProc, (LPVOID) pWatcher, 0, &nThreadID);
