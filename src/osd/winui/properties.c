...
void InitDefaultPropertyPage(HINSTANCE hInst, HWND hWnd)
{
	...
	pshead.dwFlags      = PSH_PROPSHEETPAGE | PSH_USEICONID | PSH_DEFAULT;
	...
	if (PropertySheet(&pshead) == -1)
	{
		...
		sprintf(temp, "PropertySheet creation error %d %X", (int)dwError, (int)dwError);
		win_message_box_utf8(hWnd, temp, MAMEUINAME, MB_ICONERROR | MB_OK);
	}
...
void InitPropertyPageToPage(HINSTANCE hInst, HWND hWnd, HICON hIcon, OPTIONS_TYPE opt_type, int folder_id, int game_num, int start_page )
{
	...
	pshead.hwndParent = hWnd;
	pshead.dwSize     = sizeof(PROPSHEETHEADER);
	pshead.dwFlags    = PSH_PROPSHEETPAGE | PSH_PROPTITLE | PSH_USEICONID | PSH_DEFAULT;
	pshead.hInstance  = hInst;
...
static char *GameInfoCPU(UINT nIndex)
{
	...
	while (cpu)
	{
		...
		{
			sprintf(&buf[strlen(buf)], "%s %d.%06d MHz", cpu->device().name(),
				cpu->device().clock() / 1000000, cpu->device().clock() % 1000000);
		}
		else
		{
			sprintf(&buf[strlen(buf)], "%s %d.%03d kHz", cpu->device().name(),
				cpu->device().clock() / 1000, cpu->device().clock() % 1000);
		}
...
static char *GameInfoManufactured(UINT nIndex)
{
	...
	snprintf(buffer,sizeof(buffer),"%s %s",driver_list::driver(nIndex).manufacturer,driver_list::driver(nIndex).year);
	return buffer;
}
...
char *GameInfoTitle(OPTIONS_TYPE opt_type, UINT nIndex)
{
	...
	if (OPTIONS_GLOBAL == opt_type)
		...
		sprintf(buf, "%s\n\"%s\"", driver_list::driver(nIndex).description, driver_list::driver(nIndex).name);
	return buf;
}
...
static char *GameInfoCloneOf(UINT nIndex)
{
	...
	if (DriverIsClone(nIndex))
	{
		...
		sprintf(buf, "%s - \"%s\"", ConvertAmpersandString(driver_list::driver(nParentIndex).description), 
...
static void UpdateSheetCaption(HWND hWnd)
{
	...
	if (i > 0)
	{
		...
		hFontCaption = CreateFont(-15, 0,		// height, width
	...
	rect.left = SHEET_TREE_WIDTH + 13;
	rect.top = 4;
	rect.right = rcTabCaption.right + 1;
	rect.bottom = rcTabCtrl.bottom;
...
static void AdjustChildWindows(HWND hWnd)
{
	...
	GetClassName(hWnd, (WCHAR*)szClass, sizeof(szClass));
...
static HTREEITEM GetSheetPageTreeItem(int nPage)
{
	...
	hItem = (HTREEITEM)SendMessage(hSheetTreeCtrl, TVM_GETNEXTITEM, TVGN_ROOT, (LPARAM)NULL);
	while (hItem)
	{
		item.hItem = hItem;
		item.mask = TVIF_PARAM;
		...
		hItem = (HTREEITEM)SendMessage(hSheetTreeCtrl, TVM_GETNEXTITEM, TVGN_NEXT, (LPARAM)hItem);
	}
...
static int GetSheetPageTreeCurSelText(LPWSTR lpszText, int iBufSize)
{
	...
	hItem = (HTREEITEM)SendMessage(hSheetTreeCtrl, TVM_GETNEXTITEM, TVGN_CARET, 0);
...
void ModifyPropertySheetForTreeSheet(HWND hPageDlg)
{
	...
	MovePropertySheetChildWindows(hWnd, SHEET_TREE_WIDTH + 4, -4);
	...
	hTempTab = CreateWindowEx(0, TEXT("SysTabControl32"), NULL, WS_CHILD | WS_VISIBLE | WS_CLIPSIBLINGS,
	...
	rcTabCaption.left   = rcTabCtrl.left + SHEET_TREE_WIDTH + 14;
	rcTabCaption.top    = 4;
	rcTabCaption.right  = rcTabCaption.left + (rcTabCtrl.right - rcTabCtrl.left - 8);
	rcTabCaption.bottom = rcTabCaption.top + nCaptionHeight;

	DestroyWindow(hTempTab);

	rectTree.left   = rcTabCtrl.left + 10;
	rectTree.top    = rcTabCtrl.top  + 4;
	rectTree.right  = rcTabCtrl.left + SHEET_TREE_WIDTH;
	rectTree.bottom = rcTabCtrl.bottom;

	hSheetTreeCtrl = CreateWindowEx(WS_EX_CLIENTEDGE | WS_EX_NOPARENTNOTIFY, TEXT("SysTreeView32"), TEXT("PageTree"),
						WS_TABSTOP | WS_CHILD | WS_VISIBLE | TVS_SHOWSELALWAYS | TVS_LINESATROOT | TVS_FULLROWSELECT |
						TVS_HASBUTTONS | TVS_TRACKSELECT, rectTree.left, rectTree.top,
	...
	SendMessage(hSheetTreeCtrl, TVM_SETITEMHEIGHT, 18, 0);
	...
	if (hSheetTreeCtrl == NULL)
	{
		char temp[100];
		DWORD dwError = GetLastError();
		sprintf(temp, "PropertySheet TreeCtrl creation error %d %X", (int)dwError, (int)dwError);
		win_message_box_utf8(hWnd, temp, MAMEUINAME, MB_ICONERROR | MB_OK);
	}
	...
	for (nPage = 0; nPage < nPageCount; ++nPage)
	{
		...
		// insert Item
		hItem = (HTREEITEM)SendMessage(hSheetTreeCtrl, TVM_INSERTITEM, 0, (LPARAM)&tvis);
...
INT_PTR CALLBACK GamePropertiesDialogProc(HWND hDlg, UINT Msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_INITDIALOG:
		...
		hBrushDlg = GetSysColorBrush(COLOR_WINDOW);
	...
	case WM_CTLCOLORSTATIC:
		...
		if ((HWND)lParam == GetDlgItem(hDlg, IDC_PROP_STATUS))
		{
			if (status_color == 0)
				...
				SetTextColor(hDC, RGB(202, 200, 0));
		}
		if (((HWND)lParam == GetDlgItem(hDlg, IDC_PROP_MANUFACTURED)) ||
			...
			((HWND)lParam == GetDlgItem(hDlg, IDC_PROP_COLORS)) ||
...
static void SetPropEnabledControls(HWND hWnd)
{
	...
	sound = (!core_stricmp(pCurrentOpts.value(OSDOPTION_SOUND), "dsound") 
			|| !core_stricmp(pCurrentOpts.value(OSDOPTION_SOUND), "auto"));
	d3d = (!core_stricmp(pCurrentOpts.value(OSDOPTION_VIDEO), "d3d") 
			|| !core_stricmp(pCurrentOpts.value(OSDOPTION_VIDEO), "auto"));
	...
	EnableWindow(GetDlgItem(hWnd, IDC_WIDESTRETCH), 	TRUE);
	...
	Button_Enable(GetDlgItem(hWnd, IDC_DUAL_LIGHTGUN),	TRUE);
	Button_Enable(GetDlgItem(hWnd, IDC_RELOAD),			TRUE);
...
{
	int i;
	HWND hCtrl1 = GetDlgItem(hwnd, IDC_JOYSTICK_ID_1);
	HWND hCtrl2 = GetDlgItem(hwnd, IDC_JOYSTICK_ID_2);
	HWND hCtrl3 = GetDlgItem(hwnd, IDC_JOYSTICK_ID_3);
	HWND hCtrl4 = GetDlgItem(hwnd, IDC_JOYSTICK_ID_4);
	HWND hCtrl5 = GetDlgItem(hwnd, IDC_JOYSTICK_ID_5);
	HWND hCtrl6 = GetDlgItem(hwnd, IDC_JOYSTICK_ID_6);
	HWND hCtrl7 = GetDlgItem(hwnd, IDC_JOYSTICK_ID_7);
	HWND hCtrl8 = GetDlgItem(hwnd, IDC_JOYSTICK_ID_8);

	if (hCtrl1)
	{
		for (i = 0; i < NUMJOYSTICKID; i++)
		{
			(void)ComboBox_InsertString(hCtrl1, i, g_ComboBoxJoystickID[i].m_pText);
			(void)ComboBox_SetItemData( hCtrl1, i, g_ComboBoxJoystickID[i].m_pData);
		}
	}
	if (hCtrl2)
	{
		for (i = 0; i < NUMJOYSTICKID; i++)
		{
			(void)ComboBox_InsertString(hCtrl2, i, g_ComboBoxJoystickID[i].m_pText);
			(void)ComboBox_SetItemData( hCtrl2, i, g_ComboBoxJoystickID[i].m_pData);
		}
	}
	if (hCtrl3)
	{
		for (i = 0; i < NUMJOYSTICKID; i++)
		{
			(void)ComboBox_InsertString(hCtrl3, i, g_ComboBoxJoystickID[i].m_pText);
			(void)ComboBox_SetItemData( hCtrl3, i, g_ComboBoxJoystickID[i].m_pData);
		}
	}
	if (hCtrl4)
	{
		for (i = 0; i < NUMJOYSTICKID; i++)
		{
			(void)ComboBox_InsertString(hCtrl4, i, g_ComboBoxJoystickID[i].m_pText);
			(void)ComboBox_SetItemData( hCtrl4, i, g_ComboBoxJoystickID[i].m_pData);
		}
	}
	if (hCtrl5)
	{
		for (i = 0; i < NUMJOYSTICKID; i++)
		{
			(void)ComboBox_InsertString(hCtrl5, i, g_ComboBoxJoystickID[i].m_pText);
			(void)ComboBox_SetItemData( hCtrl5, i, g_ComboBoxJoystickID[i].m_pData);
		}
	}
	if (hCtrl6)
	{
		for (i = 0; i < NUMJOYSTICKID; i++)
		{
			(void)ComboBox_InsertString(hCtrl6, i, g_ComboBoxJoystickID[i].m_pText);
			(void)ComboBox_SetItemData( hCtrl6, i, g_ComboBoxJoystickID[i].m_pData);
		}
	}
	if (hCtrl7)
	{
		for (i = 0; i < NUMJOYSTICKID; i++)
		{
			(void)ComboBox_InsertString(hCtrl7, i, g_ComboBoxJoystickID[i].m_pText);
			(void)ComboBox_SetItemData( hCtrl7, i, g_ComboBoxJoystickID[i].m_pData);
		}
	}
	if (hCtrl8)
	{
		for (i = 0; i < NUMJOYSTICKID; i++)
		{
			(void)ComboBox_InsertString(hCtrl8, i, g_ComboBoxJoystickID[i].m_pText);
			(void)ComboBox_SetItemData( hCtrl8, i, g_ComboBoxJoystickID[i].m_pData);
		}
	}
}
	...
	*filename = 0;
	if (CommonFileDialog(GetOpenFileName, filename, FILETYPE_EFFECT_FILES))
	{
		...
		if (strcmp(buff, pCurrentOpts.value(OPTION_EFFECT)))
		{
			...
			assert(!error_string);
			win_set_window_text_utf8(GetDlgItem(hWnd, IDC_EFFECT), buff);
			changed = TRUE;
		}
	}
	return changed;
...
static BOOL SelectDebugscript(HWND hWnd)
{
	...
	*filename = 0;
	if (CommonFileDialog(GetOpenFileName, filename, FILETYPE_DEBUGSCRIPT_FILES))
	{
		...
		if (strcmp(buff, pCurrentOpts.value(OPTION_DEBUGSCRIPT)))
		{
			astring error_string;
			pCurrentOpts.set_value(OPTION_DEBUGSCRIPT, buff, OPTION_PRIORITY_CMDLINE, error_string);
			assert(!error_string);
			win_set_window_text_utf8(GetDlgItem(hWnd, IDC_DEBUGSCRIPT), buff);
			changed = TRUE;
		}
	}
	return changed;
...
static BOOL ResetDebugscript(HWND hWnd)
{
	...
	if (strcmp(new_value, pCurrentOpts.value(OPTION_DEBUGSCRIPT)))
	{
		...
		assert(!error_string);
		win_set_window_text_utf8(GetDlgItem(hWnd, IDC_DEBUGSCRIPT), new_value);
		changed = TRUE;
	}
	return changed;
}
