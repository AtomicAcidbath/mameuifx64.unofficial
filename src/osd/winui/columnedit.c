	...
	void (*pfnSetColumnInfo)(int *pnOrder, int *pnShown))
{
	...
	case WM_INITDIALOG:
		...
		hHeight = ImageList_Create(1, 18, ILC_COLOR, 1, 1);
		(void)ListView_SetImageList(hShown, hHeight, LVSIL_SMALL);
		(void)ListView_SetImageList(hAvailable, hHeight, LVSIL_SMALL);
			...
			case LVN_ITEMCHANGED:
				...
				if (pnmv->uNewState & LVIS_SELECTED)
				{
					if (pnmv->iItem == 0 && pnmv->hdr.idFrom == IDC_LISTSHOWCOLUMNS)
					{
						...
						if (showMsg)
						{
							win_message_box_utf8(GetMainWindow(), "Changing this item is not permitted",
										MAMEUINAME, MB_ICONWARNING | MB_OK);
							showMsg = FALSE;
						}
