...
{
    astring fname (GetGuiDir(), PATH_SEPARATOR, "cpu.ini");
	LoadExternalFolders(parent_index, fname, IDI_CHIP);
}

void CreateSoundFolders(int parent_index)
{
    astring fname (GetGuiDir(), PATH_SEPARATOR, "sound.ini");
	LoadExternalFolders(parent_index, fname, IDI_CHIP);
}
...
{
    astring fname (GetGuiDir(), PATH_SEPARATOR, "resolution.ini");
	LoadExternalFolders(parent_index, fname, IDI_FOLDER);
}

void CreateFPSFolders(int parent_index)
{
    astring fname (GetGuiDir(), PATH_SEPARATOR, "refresh.ini");
	LoadExternalFolders(parent_index, fname, IDI_FOLDER);
}
...
static LRESULT CALLBACK TreeWndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_KEYDOWN :
		if (wParam == VK_F2)
		{
			...
			{
				SendMessage(hWnd, TVM_EDITLABEL, 0, (LPARAM)TreeView_GetSelection(hWnd));
				return TRUE;
			}
		}
		break;
